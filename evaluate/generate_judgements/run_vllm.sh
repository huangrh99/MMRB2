#!/bin/bash
# MMRB2 Benchmark - vLLM Backend Evaluation Script
#
# Usage:
#   bash run_vllm.sh <HF_MODEL_ID> [OPTIONS]
#
# Options (positional):
...
#
# Examples:
#   bash run_vllm.sh Qwen/Qwen3.5-9B                       # 1 GPU, 8 workers
#   bash run_vllm.sh Qwen/Qwen3.5-9B 16 8000 1 nothink     # nothink mode
#   bash run_vllm.sh Qwen/Qwen3.5-27B 16 8000 8             # 27B, TP=8
#   bash run_vllm.sh OpenGVLab/InternVL3_5-8B-HF 16         # InternVL
...
# Parse arguments
HF_MODEL_ID="${1:?Error: HF_MODEL_ID is required. Usage: bash run_vllm.sh <HF_MODEL_ID> [N_WORKERS] [PORT] [TP] [THINK]}"
N_WORKERS="${2:-8}"
PORT="${3:-8000}"
TP="${4:-1}"
THINK="${5:-think}"
...
mkdir -p "$LOG_DIR"
VLLM_LOG="$LOG_DIR/vllm_${SHORT_NAME}_$(date +%Y%m%d_%H%M%S).log"

export SGLANG_BASE_URL="http://localhost:${PORT}/v1"

echo "=========================================="
echo "MMRB2 Benchmark - vLLM Backend"
echo "=========================================="
...
for task_info in "1 image t2i" "2 edit edit" "3 interleaved interleaved" "4 reasoning reasoning"; do
    ...
    python multi_gpu_evaluate.py \
        ...
        --output_path "$OUTPUT_DIR/task${TASK_NUM}_vllm_${SHORT_NAME}${THINK_SUFFIX}.json" \
        --n_gpu "$N_WORKERS" \
        --n 1
